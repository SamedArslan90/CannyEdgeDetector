...
def CannyEdgeDetector(im, blur = 1.4, highThreshold = 91, lowThreshold = 31):
   ...
   #image 1
	im = imread("im01.jpg", mode="L") #Open image, convert to greyscale
	finalEdges = CannyEdgeDetector(im)
	plt.imshow(np.uint8(finalEdges)) ;
    
    #image 2
   	im = imread("im02.jpg", mode="L") #Open image, convert to greyscale
	finalEdges = CannyEdgeDetector(im)
	plt.imshow(np.uint8(finalEdges)) ;
    
    #image 3
    im = imread("im03.png", mode="L") #Open image, convert to greyscale
	finalEdges = CannyEdgeDetector(im)
	plt.imshow(np.uint8(finalEdges)) ;
